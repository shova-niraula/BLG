jQuery(document).ready(function() {
	jQuery('#mtfields input, #mtfields select, #mtfields textarea').change(function() {
...
function cc(parent_cat_id){
	if(parent_cat_id >= 0 && parent_cat_id != '') {
			jQuery.getJSON( JURI_ROOT+"?option=com_mtree&task=ajax&task2=categories.list&cat_id="+parent_cat_id+"&format=json"+(jQuery('input[name=is_admin]').val()?'&is_admin=1&no_html=1&tmpl=component':''), function(data) {
				if(data.length > 0)
				{
					clearList('browsecat');
					var c=0;
